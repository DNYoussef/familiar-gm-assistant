...
exports.CodexTheaterAuditor = void 0;
class CodexTheaterAuditor {
    constructor() {
        this.auditCounter = 0;
        this.AUDIT_INTERVAL = 3;
        this.theaterHistory = new Map();
    }
    /**
     * Increment step counter and trigger audit if needed
     */
    async checkpointStep(stepNumber, filesCreated, context) {
        this.auditCounter++;
     ...
     */
    async performTheaterAudit(files, context) {
        const results = [];
     ...
     */
    async auditFile(filePath, context) {
        const issues = [];
     ...
     */
    async fixTheaterIssues(detections) {
        ...
        for (const detection of detections) {
            ...
            const postFixValidation = await this.validateInSandbox(detection.fileAudited, 'fixed content');
     ...
     */
    async validateInSandbox(filePath, content) {
        try {
     ...
     */
    checkImplementationCompleteness(content) {
        const issues = [];
     ...
     */
    async runBasicValidation(content, filePath) {
        try {
     ...
     */
    calculatePerformanceMetrics(content, compiled) {
        if (!compiled) {
        ...
        const complexityTime = (complexityFactors.loops * 2 +
        ...
        const complexityMemory = (complexityFactors.classes * 0.1 +
...
exports.CodexTheaterAuditor = CodexTheaterAuditor;
exports.default = CodexTheaterAuditor;
