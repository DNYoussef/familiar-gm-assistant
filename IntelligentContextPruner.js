...
exports.IntelligentContextPruner = void 0;
try {
    ...
    var TfIdf = class {
        ...
        addDocument(text) { this.documents.push(text); }
        listTerms(index) { return []; }
    };
}
class IntelligentContextPruner {
    constructor(maxContextSize = 2 * 1024 * 1024) {
     ...
     */
    initializePruningStrategies() {
        this.pruningStrategies = [
            {
                name: 'temporal_relevance',
                weight: 0.25,
                calculate: (entry, context) => {
     ...
     */
    async generateSemanticVector(content) {
        try {
     ...
     */
    calculateTFIDFVector(text, docIndex, vocabulary) {
     ...
     */
    async calculateImportance(content, semanticVector) {
        const text = typeof content === 'string' ? content : JSON.stringify(content);
     ...
     */
    async calculateSemanticUniqueness(vector) {
     ...
     */
    async selectEntriesForPruning(bytesToRemove) {
     ...
     */
    identifySemanticClusters(entries) {
        ...
        for (const entry of entries) {
            ...
            for (const other of entries) {
                if (processed.has(other.id))
                    continue;
                const similarity = this.calculateCosineSimilarity(entry.semanticVector, other.semanticVector);
     ...
     */
    calculateClusterImportance(entry, clusters) {
     ...
     */
    findEntryCluster(entry, clusters) {
     ...
     */
    calculateDiversityBonus(entry, allEntries) {
     ...
     */
    calculatePruningScore(entry, context) {
     ...
     */
    async calculateDriftScore(entries) {
        if (entries.length < 2)
            return 0;
        // Calculate centroid of all semantic vectors
        const centroid = this.calculateCentroid(entries.map(e => e.semanticVector));
        // Calculate average distance from centroid
        const distances = entries.map(entry => 1 - this.calculateCosineSimilarity(entry.semanticVector, centroid));
     ...
     */
    calculateSemanticCohesion(entries) {
        if (entries.length < 2)
            return 1;
        ...
        for (let i = 0; i < entries.length; i++) {
            for (let j = i + 1; j < entries.length; j++) {
                const similarity = this.calculateCosineSimilarity(entries[i].semanticVector, entries[j].semanticVector);
     ...
     */
    calculateInformationDensity(entries) {
        if (entries.length === 0)
            return 0;
     ...
     */
    calculateTemporalRelevance(entries) {
        if (entries.length === 0)
            return 0;
     ...
     */
    calculateDomainCoverage(entries) {
...
exports.IntelligentContextPruner = IntelligentContextPruner;
