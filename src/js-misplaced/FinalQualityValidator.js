...
exports.FinalQualityValidator = void 0;
...
class FinalQualityValidator {
    constructor() {
        ...
        this.debugController = new DebugCycleController_1.DebugCycleController();
    ...
    async validateUltimate(files, context, previousEnhancement) {
        ...
        while (iteration < this.config.maxIterations && !fullyValidated) {
            ...
            const sandboxResult = await this.sandboxValidator.validateInSandbox(currentFiles, context, {
            ...
            if (!sandboxResult.success || sandboxResult.testsPassed < sandboxResult.testsTotal) {
                ...
                const debugResult = await this.debugController.runDebugCycle(currentFiles, sandboxResult, context, {
                    maxIterations: 5,
                    autoFix: true,
                    strictMode: true
                });
            ...
            if (this.isPerfect(metrics)) {
                ...
                const enhancementResult = await this.qualityEnhancer.enhanceCodeQuality(currentFiles, qualityReport, context);
    ...
    calculateMetrics(sandboxResult, qualityReport) {
    ...
    calculateCompleteness(sandboxResult, qualityReport) {
        const factors = [];
        // Code coverage
    ...
    calculateNasaCompliance(report) {
        if (!report.nasaCompliance)
            return 0;
        const totalRules = 10;
    ...
    calculateDefenseCompliance(report) {
        if (!report.defenseStandards)
            return 0;
        const factors = [
    ...
    calculateEnterpriseCompliance(report) {
        if (!report.enterpriseStandards)
            return 0;
        const factors = [
    ...
    isPerfect(metrics) {
        ...
        const perfect = metrics.completeness === 100 &&
    ...
    logDeficiencies(metrics, certification) {
...
exports.FinalQualityValidator = FinalQualityValidator;
