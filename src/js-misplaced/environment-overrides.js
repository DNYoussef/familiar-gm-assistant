...
exports.EnvironmentOverrideSystem = void 0;
const zod_1 = require("zod");
// Environment variable mapping schema
const EnvironmentMappingSchema = zod_1.z.object({
    ...
    validation: zod_1.z.function().optional(),
    transformation: zod_1.z.function().optional(),
    deprecated: zod_1.z.boolean().default(false),
    deprecationMessage: zod_1.z.string().optional(),
    secretHandling: zod_1.z.object({
        isSecret: zod_1.z.boolean().default(false),
        vaultPath: zod_1.z.string().optional(),
        rotationRequired: zod_1.z.boolean().default(false)
    }).optional()
});
/**
 * Environment Variable Override System
 ...
 */
class EnvironmentOverrideSystem {
    constructor(config = {}) {
    ...
    initializeStandardMappings() {
        const mappings = [
            ...
            },
            {
                ...
                type: 'string',
                validation: (value) => ['community', 'professional', 'enterprise'].includes(value)
            },
            {
                configPath: 'enterprise.compliance_level',
                environmentKey: 'ENTERPRISE_CONFIG_COMPLIANCE_LEVEL',
                type: 'string',
                validation: (value) => ['standard', 'strict', 'nasa-pot10', 'defense'].includes(value)
            },
            ...
            },
            {
                ...
                type: 'string',
                validation: (value) => ['basic', 'oauth2', 'saml', 'ldap', 'multi_factor'].includes(value)
            },
            {
            ...
            },
            {
                ...
                type: 'number',
                validation: (value) => value >= 30 && value <= 365
            },
            ...
            // Database configuration
            {
                ...
                type: 'number',
                validation: (value) => value >= 5 && value <= 100
            },
            // API configuration
            {
                ...
                type: 'number',
                validation: (value) => value >= 10 && value <= 10000
            },
            // Monitoring settings
            {
            ...
            },
            {
                ...
                type: 'number',
                validation: (value) => value >= 10 && value <= 300
            },
            ...
            // Quality gates
            {
                ...
                type: 'number',
                validation: (value) => value >= 0.0 && value <= 1.0
            },
            {
     ...
     */
    async processEnvironmentVariable(envKey, mapping) {
     ...
     * Process dynamic environment variables with prefix
     */
    async processDynamicEnvironmentVariables() {
        const overrides = {};
        const secrets = {};
        const warnings = [];
        const errors = [];
     ...
     */
    async processSecret(key, value, mapping) {
        const metadata = {
            isSecret: true,
            source: 'environment',
        ...
        if (this.config.secretHandling.secretRotation.enabled &&
            ...
            metadata.rotationScheduled = new Date(Date.now() + this.config.secretHandling.secretRotation.intervalHours * 60 * 60 * 1000);
        }
    ...
    assessSecretStrength(secret) {
        ...
        if (criteria >= 3 && secret.length >= 16)
            return 'strong';
        if (criteria >= 2 && secret.length >= 12)
            return 'medium';
        return 'weak';
    }
    /**
     ...
     */
    async fetchSecretFromVault(vaultPath) {
        if (!this.vaultClient) {
            return null;
        }
        try {
     ...
     */
    async validateOverrides(overrides) {
        const warnings = [];
        const errors = [];
     ...
     */
    applyOverridesToConfig(config, overrides) {
...
exports.EnvironmentOverrideSystem = EnvironmentOverrideSystem;
exports.default = EnvironmentOverrideSystem;
//# sourceMappingURL=environment-overrides.js.map