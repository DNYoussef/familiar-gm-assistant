...
class ContextRouter extends events_1.EventEmitter {
    constructor(princesses, compressionThreshold = 1024) {
        super();
        ...
        this.monitor = new DegradationMonitor_1.DegradationMonitor();
     ...
     */
    async routeContext(context, sourcePrincess, options = {}) {
        ...
        const targets = await this.selectTargetPrincesses(analysis, sourcePrincess, options.excludePrincesses || []);
     ...
     */
    async selectTargetPrincesses(analysis, sourcePrincess, exclude) {
        const candidates = [];
        for (const [princessId, capabilities] of this.routingTable) {
            ...
            const score = this.calculateRoutingScore(capabilities, analysis, princessId);
            if (score > 0.3) {
     ...
     */
    calculateRoutingScore(capabilities, analysis, princessId) {
        let score = 0;
        // Domain relevance (40%)
     ...
     */
    async executeRouting(decision, context) {
     ...
     */
    async targetedRoute(decision, context) {
        for (const target of decision.targetPrincesses) {
            try {
                ...
                const customContext = await this.customizeForDomain(context, this.routingTable.get(target)?.domains || []);
     ...
     */
    async cascadeRoute(decision, context) {
        ...
        for (const target of decision.targetPrincesses) {
            try {
                ...
                const degradation = await this.monitor.detectDegradation(context, currentContext);
                if (degradation > 0.15) {
     ...
     */
    async redundantRoute(decision, context) {
        ...
        const primaryResults = await Promise.allSettled(primaryTargets.map(target => this.routeToPrincess(target, context, decision)));
        ...
        if (primarySuccessRate < 0.7) {
            ...
            await Promise.allSettled(backupTargets.map(target => this.routeToPrincess(target, context, decision)));
        }
    }
    /**
     * Route to individual princess
     */
    async routeToPrincess(target, context, decision) {
        const princess = this.princesses.get(target);
        if (!princess)
            throw new Error(`Princess ${target} not found`);
        await princess.handleContext(context, {
     ...
     * Prepare context for routing
     */
    async prepareContext(context, decision) {
     ...
     */
    async customizeForDomain(context, domains) {
     ...
     */
    initializeCircuitBreaker(princess) {
    ...
    determineStrategy(targetCount, complexity) {
        if (targetCount >= 4)
            return 'broadcast';
        if (complexity > 0.7)
            return 'cascade';
    ...
    detectContextType(context) {
        if (context.code || context.implementation)
            return 'code';
        if (context.tests || context.coverage)
            return 'test';
        if (context.auth || context.permissions)
            return 'security';
        if (context.deployment || context.pipeline)
            return 'infrastructure';
        if (context.research || context.analysis)
            return 'research';
        if (context.plan || context.coordination)
            return 'coordination';
        return 'general';
    }
    ...
    getMetrics() {
        ...
        const successful = this.routingHistory.filter(d => d.targetPrincesses.length > 0).length;
        return {
            totalRouted: total,
     ...
     */
    async discoverPrincessCapabilities(princessId, princess) {
        try {
     ...
     */
    calculateReliability(princessId, metrics) {
        const breaker = this.circuitBreakers.get(princessId);
        if (!breaker)
            return 0.95;
...
class NLPProcessor {
     ...
     */
    async extractDomainRelevance(nlpAnalysis) {
     ...
     */
    calculateEntityBoost(entities, domain) {
        ...
        const matches = entities.filter(entity => relevantEntities.some(relevant => entity.toLowerCase().includes(relevant.toLowerCase())));
