...
exports.ResultCorrelationFramework = void 0;
 ...
 */
class ResultCorrelationFramework extends events_1.EventEmitter {
    constructor(analysisContext) {
        super();
        this.analysisContext = analysisContext;
        this.correlationRules = new Map();
        this.connascencePatterns = new Map();
        this.violationClusters = new Map();
        this.correlationHistory = [];
        this.confidenceThreshold = 0.75;
        this.maxCorrelationDistance = 10; // lines
     ...
     */
    async discoverBasicCorrelations(results) {
        const correlations = [];
     ...
     */
    async calculateDetailedCorrelation(resultA, resultB) {
     ...
     */
    calculateAdvancedViolationSimilarity(violationA, violationB) {
        let score = 0;
     ...
     */
    calculateLocationSimilarity(violationA, violationB) {
     ...
     */
    calculateSeveritySimilarity(violationA, violationB) {
     ...
     */
    calculateMessageSimilarity(messageA, messageB) {
     ...
     */
    calculateRuleSimilarity(ruleA, ruleB) {
        if (ruleA === ruleB)
            return 1.0;
     ...
     */
    async analyzeConnascencePatterns(results) {
        const analyses = [];
     ...
     */
    identifyConnascencePatterns(violation) {
        const patterns = [];
     ...
     */
    async formViolationClusters(results, correlations) {
        const clusters = [];
     ...
     */
    async identifyAndResolveConflicts(results) {
        const conflicts = [];
     ...
     */
    async integrateWithConnascenceAnalysis(clusters) {
    ...
    // Helper methods
    getApplicableRules(toolA, toolB) {
        return Array.from(this.correlationRules.values()).filter(rule => rule.enabled &&
            rule.sourceTools.includes(toolA) &&
            rule.sourceTools.includes(toolB));
    }
    identifyDetailedCorrelationPattern(violationPairs, resultA, resultB) {
    ...
    findCommonCategories(resultA, resultB) {
    ...
    calculateCorrelationMetrics(correlations, clusters) {
    ...
    findViolationById(results, violationId) {
        for (const result of results) {
    ...
    calculateConnascenceSeverity(patterns) {
    ...
    calculateClusterSeverity(violations) {
    ...
    determineClusterCategory(violations) {
    ...
    identifyRootCause(violations, patterns) {
    ...
    generateConnascenceRecommendations(patterns) {
    ...
    generateClusterRecommendations(violations, patterns) {
    ...
    generateIntegrationRecommendations(clusters) {
        const recommendations = [];
    ...
    groupViolationsByLocation(results) {
    ...
    async resolveViolationConflict(violations) {
...
exports.ResultCorrelationFramework = ResultCorrelationFramework;
//# sourceMappingURL=result-correlation-framework.js.map