...
class PerformanceMonitor extends events_1.EventEmitter {
     ...
     */
    async detectRegressions(artifacts, context) {
        const violations = [];
        ...
        try {
            ...
            const performanceRecommendations = this.generatePerformanceRecommendations(currentMetrics, regression, alerts);
     ...
     * Extract performance data from artifacts
     */
    async extractPerformanceData(artifacts, context) {
        const data = {};
     ...
     * Calculate response time metrics
     */
    calculateResponseTimeMetrics(data) {
     ...
     * Calculate throughput metrics
     */
    calculateThroughputMetrics(data) {
     ...
     * Calculate availability metrics
     */
    calculateAvailabilityMetrics(data) {
     ...
     */
    async analyzeRegression(currentMetrics, baseline) {
        const baselineMetrics = baseline.metrics;
        const regressions = [];
        // Check response time regression
        const responseTimeRegression = this.calculateRegressionPercentage(currentMetrics.responseTime.mean, baselineMetrics.responseTime.mean);
        ...
        const throughputRegression = this.calculateRegressionPercentage(baselineMetrics.throughput.requestsPerSecond, currentMetrics.throughput.requestsPerSecond);
        ...
        const errorRateRegression = this.calculateRegressionPercentage(currentMetrics.errors.errorRate, baselineMetrics.errors.errorRate);
        ...
        const severity = this.categorizeRegressionSeverity(maxRegression);
     ...
     */
    calculateRegressionPercentage(current, baseline) {
        if (baseline === 0)
            return 0;
     ...
     */
    categorizeRegressionSeverity(percentage) {
        if (percentage >= 20)
            return 'critical';
        if (percentage >= 10)
            return 'high';
     ...
     */
    generateImpactAssessment(regressions) {
     ...
     */
    identifyRootCauses(current, baseline, regressions) {
        const rootCauses = [];
     ...
     * Generate performance alerts
     */
    async generatePerformanceAlerts(metrics, regression) {
        const alerts = [];
     ...
     */
    storeHistoricalData(metrics, context) {
     ...
     */
    generatePerformanceRecommendations(metrics, regression, alerts) {
        const recommendations = [];
     ...
     */
    extractLoadTestData(artifacts) {
        return artifacts.reduce((acc, artifact) => ({
            ...acc,
            ...artifact.data
        }), {});
    }
    extractApplicationMetrics(artifacts) {
        return artifacts.reduce((acc, artifact) => ({
            ...acc,
            ...artifact.data
        }), {});
    }
    extractInfrastructureMetrics(artifacts) {
        return artifacts.reduce((acc, artifact) => ({
            ...acc,
            ...artifact.data
        }), {});
    }
    extractAPMData(artifacts) {
        return artifacts.reduce((acc, artifact) => ({
            ...acc,
            ...artifact.data
        }), {});
    }
    extractSyntheticData(artifacts) {
        return artifacts.reduce((acc, artifact) => ({
            ...acc,
            ...artifact.data
        }), {});
     ...
     */
    setBaseline(version, environment, metrics, testConfiguration) {
        const baseline = {
            timestamp: new Date(),
            version,
            environment,
     ...
     */
    getPerformanceTrends(environment = 'default') {
