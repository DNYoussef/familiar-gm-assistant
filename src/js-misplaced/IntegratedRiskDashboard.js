...
exports.IntegratedRiskDashboard = void 0;
const react_1 = __importStar(require("react"));
const RiskMonitoringDashboard_1 = require("./RiskMonitoringDashboard");
const GaryDPIEngine_1 = __importDefault(require("./GaryDPIEngine"));
const TalebBarbellEngine_1 = __importDefault(require("./TalebBarbellEngine"));
const KellyCriterionEngine_1 = __importDefault(require("./KellyCriterionEngine"));
 ...
 */
const IntegratedRiskDashboard = ({ className = '' }) => {
    // System engines
    const garyEngine = (0, react_1.useRef)(null);
    const talebEngine = (0, react_1.useRef)(null);
    const kellyEngine = (0, react_1.useRef)(null);
    const riskMonitor = (0, react_1.useRef)(null);
    // Dashboard state
    ...
    // Data state
    const [garyData, setGaryData] = (0, react_1.useState)({ signals: [], condition: {}, portfolio: {}, recommendations: [] });
    const [talebData, setTalebData] = (0, react_1.useState)({ allocation: {}, regime: {}, rebalanceRec: null });
    const [kellyData, setKellyData] = (0, react_1.useState)({ portfolio: {}, positions: [], opportunities: [] });
    const [riskData, setRiskData] = (0, react_1.useState)({ metrics: null, alerts: [], dashboardState: null });
    /**
    ...
    (0, react_1.useEffect)(() => {
        const initializeSystems = async () => {
            try {
                ...
                garyEngine.current = new GaryDPIEngine_1.default();
                setupGaryEventHandlers();
                // Initialize Taleb's Barbell Engine (Phase 2)
                talebEngine.current = new TalebBarbellEngine_1.default();
                setupTalebEventHandlers();
                // Initialize Kelly Criterion Engine (Phase 2)
                kellyEngine.current = new KellyCriterionEngine_1.default();
                setupKellyEventHandlers();
                // Initialize Risk Monitoring Dashboard
                riskMonitor.current = new RiskMonitoringDashboard_1.RiskMonitoringDashboard();
        ...
        // Cleanup on unmount
        return () => {
            if (garyEngine.current)
                garyEngine.current.stop();
            if (talebEngine.current)
                talebEngine.current.stop();
            if (kellyEngine.current)
                kellyEngine.current.stop();
     ...
     */
    const setupGaryEventHandlers = (0, react_1.useCallback)(() => {
        if (!garyEngine.current)
            return;
        const engine = garyEngine.current;
        engine.on('signals', (signals) => {
     ...
     */
    const setupTalebEventHandlers = (0, react_1.useCallback)(() => {
        if (!talebEngine.current)
            return;
        ...
        engine.on('rebalanceRecommendation', (recommendation) => {
     ...
     */
    const setupKellyEventHandlers = (0, react_1.useCallback)(() => {
        if (!kellyEngine.current)
            return;
     ...
     */
    const setupRiskEventHandlers = (0, react_1.useCallback)(() => {
        if (!riskMonitor.current)
            return;
    ...
    return (<div className={`min-h-screen bg-gray-100 ${className}`}>
          ...
          <div className="flex items-center justify-between h-16">
            ...
            <div className="flex items-center space-x-6">
              ...
              <div className="flex items-center text-sm">
                <div className={`w-3 h-3 rounded-full mr-2 ${systemHealth === 'HEALTHY' ? 'bg-green-500' :
      ...
      <main className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-6">
        <div className="space-y-6">
          ...
          {/* Main P(ruin) Display */}
          {riskData.metrics && (<div className="bg-white rounded-lg shadow-lg p-8">
              <h2 className="text-2xl font-bold text-gray-800 mb-6 text-center">
                Real-Time Probability of Ruin
              </h2>
              <div className="text-center">
                <div className="text-6xl font-bold mb-4" style={{ color: riskData.metrics.pRuin.value > 0.1 ? COLORS.RISK.CRITICAL :
          ...
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
            ...
            <div className="space-y-6">
              <div className="bg-white rounded-lg shadow-lg p-6">
                ...
                {garyData.signals.length > 0 ? (<div className="space-y-3">
                    {garyData.signals.slice(-5).map((signal, idx) => (<div key={idx} className="border rounded-lg p-3">
                        <div className="flex items-center justify-between mb-2">
                          <span className="font-medium">{signal.type}</span>
                          <span className={`px-2 py-1 rounded text-xs font-medium text-white ${signal.riskLevel === 'CRITICAL' ? 'bg-red-500' :
              ...
              {/* Market Condition */}
              {garyData.condition.timestamp && (<div className="bg-white rounded-lg shadow p-4">
            ...
            <div className="space-y-6">
              <div className="bg-white rounded-lg shadow-lg p-6">
                ...
                {talebData.allocation.timestamp ? (<div className="space-y-4">
                    ...
                    <div className="space-y-3">
                      ...
                      <div className="w-full bg-gray-200 rounded-full h-3">
                        <div className="bg-green-500 h-3 rounded-full" style={{ width: `${talebData.allocation.safeAssets.allocation * 100}%` }}></div>
                      </div>
                      ...
                      <div className="w-full bg-gray-200 rounded-full h-3">
                        <div className="bg-red-500 h-3 rounded-full" style={{ width: `${talebData.allocation.riskAssets.allocation * 100}%` }}></div>
                      </div>
                    </div>
                    ...
                    {/* Rebalance Signal */}
                    {talebData.allocation.rebalanceSignal !== 'NONE' && (<div className={`p-3 rounded-lg ${talebData.allocation.rebalanceSignal === 'CRITICAL' ? 'bg-red-100 border border-red-300' :
              ...
              {/* Market Regime */}
              {talebData.regime.timestamp && (<div className="bg-white rounded-lg shadow p-4">
                  <h4 className="font-semibold mb-3">Market Regime</h4>
                  <div className="space-y-2 text-sm">
                    <div className="flex justify-between">
                      <span className="text-gray-600">Regime:</span>
                      <span className={`font-medium px-2 py-1 rounded text-xs ${talebData.regime.regime === 'CRISIS' ? 'bg-red-100 text-red-800' :
            ...
            <div className="space-y-6">
              <div className="bg-white rounded-lg shadow-lg p-6">
                ...
                {kellyData.positions.length > 0 ? (<div className="space-y-3">
                    {kellyData.positions.slice(0, 5).map((position, idx) => (<div key={idx} className="border rounded-lg p-3">
                        <div className="flex items-center justify-between mb-2">
                          <span className="font-medium">{position.asset}</span>
                          <span className={`px-2 py-1 rounded text-xs font-medium text-white ${position.urgency === 'CRITICAL' ? 'bg-red-500' :
              ...
              {kellyData.portfolio.timestamp && (<div className="bg-white rounded-lg shadow p-4">
          ...
          {/* Active Alerts */}
          {riskData.alerts.length > 0 && (<div className="bg-white rounded-lg shadow-lg p-6">
              <h3 className="text-lg font-semibold text-gray-800 mb-4">
                 Active Risk Alerts
              </h3>
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                {riskData.alerts.slice(-6).map((alert) => (<div key={alert.id} className={`border-l-4 p-4 rounded-r-lg ${alert.acknowledged ? 'bg-gray-50 opacity-75' : 'bg-white'}`} style={{ borderLeftColor: COLORS.RISK[alert.type] }}>
                    <div className="flex items-center justify-between mb-2">
                      <span className="px-2 py-1 text-xs font-medium text-white rounded-full" style={{ backgroundColor: COLORS.RISK[alert.type] }}>
                        {alert.type}
                      </span>
...
exports.IntegratedRiskDashboard = IntegratedRiskDashboard;
exports.default = exports.IntegratedRiskDashboard;
//# sourceMappingURL=IntegratedRiskDashboard.js.map