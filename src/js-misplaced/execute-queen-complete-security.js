...
╚══════════════════════════════════════════════════════════════════════════════╝
    ...
    async fixWeakHash(file) {
        const fullPath = path.join(process.cwd(), '../../', file);
        if (fs.existsSync(fullPath)) {
            let content = fs.readFileSync(fullPath, 'utf-8');
            content = content.replace(/hashlib\.(md5|sha1)\((.*?)\)(?!.*usedforsecurity)/g, 'hashlib.$1($2, usedforsecurity=False)');
            fs.writeFileSync(fullPath, content);
        }
    }
    async updateConfiguration(file) {
