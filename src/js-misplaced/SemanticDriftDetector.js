...
class SemanticDriftDetector {
     ...
     */
    async generateSemanticVector(context) {
        try {
    ...
    calculateVelocity(snapshots) {
        ...
        for (let i = 1; i < snapshots.length; i++) {
            ...
            if (timeDelta > 0) {
                const semanticDistance = this.calculateSemanticDistance(snapshots[i - 1].semanticVector, snapshots[i].semanticVector);
     ...
     */
    calculateAcceleration(snapshots) {
     ...
     */
    calculateDriftDirection(snapshots) {
        if (snapshots.length < 2)
            return [];
    ...
    calculateCoherence(snapshots) {
        ...
        for (let i = 0; i < snapshots.length; i++) {
            for (let j = i + 1; j < snapshots.length; j++) {
                const similarity = this.calculateCosineSimilarity(snapshots[i].semanticVector, snapshots[j].semanticVector);
     ...
     */
    calculatePredictability(snapshots) {
        if (snapshots.length < 3)
            return 0;
        ...
        for (let i = 1; i < snapshots.length; i++) {
            const timeDelta = snapshots[i].timestamp - snapshots[i - 1].timestamp;
            if (timeDelta > 0) {
                const distance = this.calculateSemanticDistance(snapshots[i - 1].semanticVector, snapshots[i].semanticVector);
     ...
     */
    async identifyDriftPatterns(metrics) {
        const patterns = [];
    ...
    detectOscillation(metrics) {
        try {
            ...
            for (let i = 1; i < recentSnapshots.length; i++) {
                ...
                if (timeDelta > 0) {
                    const distance = this.calculateSemanticDistance(recentSnapshots[i - 1].semanticVector, recentSnapshots[i].semanticVector);
     ...
     */
    async updateAdaptiveThresholds(metrics) {
        const metricValues = {
    ...
    calculateSemanticDistance(vec1, vec2) {
    ...
    getObjectDepth(obj, currentDepth = 0) {
        ...
        return Math.max(currentDepth, ...Object.values(obj).map(v => this.getObjectDepth(v, currentDepth + 1)));
    }
    /**
...
//# sourceMappingURL=SemanticDriftDetector.js.map