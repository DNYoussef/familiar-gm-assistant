...
const ContextDNA_1 = require("./ContextDNA");
class ContextValidator {
    constructor() {
        this.validationHistory = new Map();
        this.planeProjectId = null;
     ...
     */
    async initializePlaneConnection() {
        try {
            // Real MCP connection attempt
            if (typeof globalThis !== 'undefined' && globalThis.mcp__plane__connect) {
                const connectionResult = await globalThis.mcp__plane__connect({
     ...
     */
    async createValidationProject() {
        try {
            if (typeof globalThis !== 'undefined' && globalThis.mcp__plane__createProject) {
                await globalThis.mcp__plane__createProject({
     ...
     */
    async initializeLocalValidation() {
     ...
     */
    async validateContext(context, fingerprint, validationGate) {
     ...
     */
    async validateProcessTruth(context, fingerprint, gate) {
        const details = [];
        let score = 100;
     ...
     */
    async validateSemanticTruth(context, fingerprint, gate) {
        const details = [];
        let score = 100;
     ...
     */
    async validateIntegrityTruth(context, fingerprint, gate) {
        const details = [];
        // Use Context DNA validation
        const dnaValidation = ContextDNA_1.ContextDNA.validateTransfer(fingerprint, context, fingerprint.targetAgent);
        let score = 100;
     ...
     */
    generateRecommendations(layers, degradationLevel, gate) {
     ...
     */
    async triggerValidationAction(action, validation, context) {
     ...
     */
    async checkPlaneRecord(fingerprint) {
        try {
            // Attempt real MCP call
            if (typeof globalThis !== 'undefined' && globalThis.mcp__plane__getIssue) {
                const issue = await globalThis.mcp__plane__getIssue(fingerprint.checksum);
                if (issue) {
                    return {
                        exists: true,
                        issueId: issue.id,
    ...
    async validateTaskBoundary(taskId) {
        try {
            ...
            if (typeof globalThis !== 'undefined' && globalThis.mcp__plane__getIssue) {
                try {
                    const task = await globalThis.mcp__plane__getIssue(taskId);
                    if (!task) {
                        return {
                            valid: false,
    ...
    async verifyAuditTrail(fingerprint) {
        try {
            const auditDetails = [];
            let complete = true;
            ...
            if (typeof globalThis !== 'undefined' && globalThis.mcp__plane__searchIssues) {
                try {
                    const auditIssues = await globalThis.mcp__plane__searchIssues({
                        projectId: this.planeProjectId,
     ...
     */
    async calculateSemanticScore(context, fingerprint) {
        try {
     ...
     */
    async vectorBasedSemanticScore(context, fingerprint) {
        if (fingerprint.semanticVector && fingerprint.semanticVector.length > 0) {
            const currentVector = this.generateSemanticVector(context);
            const similarity = this.calculateCosineSimilarity(fingerprint.semanticVector, currentVector);
     ...
     */
    async structuralSemanticScore(context, fingerprint) {
     ...
     */
    async contentHashSemanticScore(context, fingerprint) {
        const contextStr = JSON.stringify(context);
    ...
    extractKeywords(text) {
        ...
        const filtered = words.filter(word => !['true', 'false', 'null', 'undefined'].includes(word.toLowerCase()));
    ...
    extractStructure(obj) {
        ...
        return {
            ...
            structure: Object.fromEntries(Object.keys(obj).map(key => [key, this.extractStructure(obj[key])]))
        };
    }
    /**
     * Extract structure from fingerprint
     */
    extractStructureFromFingerprint(fingerprint) {
     ...
     */
    compareStructures(current, original) {
     ...
     */
    generateSemanticVector(context) {
    ...
    async validateEntityRelationships(context) {
        try {
            ...
            if (typeof globalThis !== 'undefined' && globalThis.mcp__memory__open_nodes) {
                try {
                    const entityNames = entities.map(e => e.name);
                    const nodeResults = await globalThis.mcp__memory__open_nodes({
                        names: entityNames
                    });
                    // Check which entities exist in memory
                    const existingEntities = new Set(nodeResults.map((n) => n.name));
    ...
    async checkKnowledgeGraphConsistency(context) {
        try {
            // Attempt real knowledge graph consistency check
            if (typeof globalThis !== 'undefined' && globalThis.mcp__memory__read_graph) {
                try {
                    const graph = await globalThis.mcp__memory__read_graph({});
            ...
            if (context && typeof context === 'object') {
                ...
                const foundPatterns = keyPatterns.filter(pattern => contextStr.toLowerCase().includes(pattern));
     ...
     */
    async escalateToQueen(validation, context) {
        try {
    ...
    async initiateRecovery(validation, context) {
        try {
    ...
    async alertPrincesses(validation) {
        try {
            ...
            if (typeof globalThis !== 'undefined' && globalThis.mcp__plane__createIssue) {
                ...
                for (const domain of princessDomains) {
                    alertPromises.push(globalThis.mcp__plane__createIssue({
                        projectId: this.planeProjectId,
            ...
            if (typeof globalThis !== 'undefined' && globalThis.mcp__memory__create_entities) {
                alertPromises.push(globalThis.mcp__memory__create_entities({
                    entities: [{
     ...
     */
    getValidationHistory(agentPair) {
        if (agentPair) {
     ...
     */
    calculateCumulativeDegradation(transferChain) {
        ...
        for (let i = 0; i < transferChain.length - 1; i++) {
            ...
            if (history && history.length > 0) {
                ...
                cumulativeDegradation = Math.max(cumulativeDegradation, latestValidation.degradationLevel);
            }
        }
...
ContextValidator.VALIDATION_GATES = {
    preTransfer: {
...
exports.default = ContextValidator;
