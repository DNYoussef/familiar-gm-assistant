...
exports.CanaryController = void 0;
...
class CanaryController {
     ...
     */
    async progressCanary(deploymentId, targetPercentage) {
     ...
     */
    async pauseCanary(deploymentId, reason) {
        const canaryDeployment = this.canaryDeployments.get(deploymentId);
        if (!canaryDeployment) {
            throw new Error(`Canary deployment ${deploymentId} not found`);
        }
     ...
     */
    async initializeCanaryDeployment(execution) {
     ...
     */
    async deployCanaryVersion(canaryDeployment) {
     ...
     */
    async executeProgressiveRollout(canaryDeployment) {
        canaryDeployment.status = 'progressing';
     ...
     */
    async completeCanaryDeployment(canaryDeployment, progressResult) {
     ...
     * Validate progression request
     */
    async validateProgressionRequest(canaryDeployment, targetPercentage) {
    ...
    async executeCanaryDeployment(canaryDeployment) {
        const orchestrator = new container_orchestrator_1.ContainerOrchestrator(canaryDeployment.execution.environment);
        const config = canaryDeployment.config;
        try {
            // Deploy canary instances (subset of total replicas)
            const canaryReplicas = Math.ceil(canaryDeployment.execution.replicas * (config.initialTrafficPercentage / 100));
            const deployResult = await orchestrator.deployContainers(canaryDeployment.execution.artifact, `${canaryDeployment.execution.environment.namespace}-canary`, canaryReplicas);
            if (!deployResult.success) {
                throw new Error(`Canary deployment failed: ${deployResult.error}`);
            }
            // Wait for canary containers to be ready
            await orchestrator.waitForContainerReadiness(`${canaryDeployment.execution.environment.namespace}-canary`, canaryReplicas, 120000 // 2 minutes timeout
            );
    ...
    async waitForCanaryReadiness(canaryDeployment) {
    ...
    async checkCanaryHealth(canaryDeployment) {
    ...
    calculateSuccessMetrics(canaryDeployment, duration) {
        return {
    ...
    calculatePartialSuccessMetrics(canaryDeployment, duration) {
...
class MetricsCollector {
    ...
    async collectEnvironmentMetrics(namespace) {
...
class ProgressMonitor {
    constructor(canaryDeployment, metricsCollector) {
        this.canaryDeployment = canaryDeployment;
    ...
    async executeProgressiveRollout() {
        ...
        try {
            while (currentPercentage < 100 && stepNumber <= config.maxSteps) {
                ...
                const nextPercentage = Math.min(currentPercentage + config.stepPercentage, 100);
                // Execute step
    ...
    async progressToPercentage(targetPercentage) {
    ...
    async executeStep(stepNumber, fromPercentage, toPercentage) {
    ...
    async shiftTraffic(fromPercentage, toPercentage) {
        const loadBalancer = new load_balancer_manager_1.LoadBalancerManager(this.canaryDeployment.execution.environment);
    ...
    async monitorStepHealth(stepNumber) {
        const config = this.canaryDeployment.config;
    ...
    checkSuccessCriteria(metrics) {
        ...
        return (metrics.errorRate <= successThreshold.errorRate &&
