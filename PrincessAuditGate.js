...
exports.PrincessAuditGate = void 0;
const events_1 = require("events");
const CodexTheaterAuditor_1 = require("./CodexTheaterAuditor");
const CodexSandboxValidator_1 = require("./CodexSandboxValidator");
const DebugCycleController_1 = require("./DebugCycleController");
const GitHubCompletionRecorder_1 = require("./GitHubCompletionRecorder");
const ContextDNA_1 = require("../../context/ContextDNA");
const EnterpriseQualityAnalyzer_1 = require("./EnterpriseQualityAnalyzer");
const CodexQualityEnhancer_1 = require("./CodexQualityEnhancer");
const FinalQualityValidator_1 = require("./FinalQualityValidator");
class PrincessAuditGate extends events_1.EventEmitter {
    constructor(princessDomain, config) {
        super();
        this.princessDomain = princessDomain;
        ...
        this.theaterAuditor = new CodexTheaterAuditor_1.CodexTheaterAuditor();
        this.sandboxValidator = new CodexSandboxValidator_1.CodexSandboxValidator();
        this.debugController = new DebugCycleController_1.DebugCycleController(this.config.maxDebugIterations);
        this.githubRecorder = new GitHubCompletionRecorder_1.GitHubCompletionRecorder();
        this.qualityAnalyzer = new EnterpriseQualityAnalyzer_1.EnterpriseQualityAnalyzer();
        this.qualityEnhancer = new CodexQualityEnhancer_1.CodexQualityEnhancer();
        this.finalValidator = new FinalQualityValidator_1.FinalQualityValidator();
    ...
    async auditSubagentWork(work) {
        ...
        try {
            ...
            const enhancementResult = await this.qualityEnhancer.enhanceCodeQuality(work.files, qualityReport, work.context);
            ...
            const ultimateValidation = await this.finalValidator.validateUltimate(work.files, work.context, enhancementResult);
     ...
     */
    async performTheaterDetection(work, auditResult) {
        const detections = [];
     ...
     */
    async performSandboxValidation(work, auditResult) {
        ...
        const sandboxResult = await this.sandboxValidator.validateInSandbox(work.files, work.context, sandboxConfig);
     ...
     */
    async performDebugCycle(work, auditResult, sandboxResult) {
        console.log(`  Starting debug cycle (max ${this.config.maxDebugIterations} iterations)`);
        const debugResult = await this.debugController.runDebugCycle(work.files, sandboxResult, work.context, {
            subagentId: work.subagentId,
            taskId: work.taskId,
            model: 'gpt-5-codex'
        });
     ...
     * Stage 4: Final Validation
     */
    async performFinalValidation(work, auditResult) {
        const issues = [];
        ...
        const contextFingerprint = ContextDNA_1.ContextDNA.generateFingerprint(work.context, work.subagentId, this.princessDomain);
     ...
     */
    async rejectWithTheater(work, auditResult) {
        ...
        if (auditResult.theaterDetails) {
            for (const detection of auditResult.theaterDetails) {
                if (detection.theaterFound) {
                    auditResult.reworkInstructions.push(`File: ${detection.fileAudited}`, `  Theater Type: ${detection.theaterType}`, `  Issues to fix:`);
                    for (const issue of detection.issues) {
                        auditResult.reworkInstructions.push(`    Line ${issue.line}: ${issue.description}`, `      Fix: ${issue.suggestedFix}`);
                    }
                }
            }
        }
     ...
     */
    async rejectWithDebugFailure(work, auditResult) {
     ...
     */
    async rejectWithValidationFailure(work, auditResult, validation) {
     ...
     */
    async rejectWithQualityFailure(work, auditResult, validation) {
        ...
        if (validation.metrics.completeness < 100) {
            auditResult.reworkInstructions.push(`- Completeness issues: Implement all missing functionality`, `  Current: ${validation.metrics.completeness.toFixed(1)}%`);
        }
        if (validation.metrics.functionality < 100) {
            auditResult.reworkInstructions.push(`- Functionality issues: Fix all test failures and runtime errors`, `  Current: ${validation.metrics.functionality.toFixed(1)}%`);
        }
        if (validation.metrics.quality < 100) {
            auditResult.reworkInstructions.push(`- Quality issues: Address all code quality violations`, `  Current: ${validation.metrics.quality.toFixed(1)}%`);
        }
        if (validation.metrics.nasaCompliance < 100) {
            auditResult.reworkInstructions.push(`- NASA compliance: Implement all Power of Ten rules`, `  Current: ${validation.metrics.nasaCompliance.toFixed(1)}%`);
        }
        if (validation.metrics.defenseCompliance < 100) {
            auditResult.reworkInstructions.push(`- Defense standards: Meet all DFARS/MIL-STD requirements`, `  Current: ${validation.metrics.defenseCompliance.toFixed(1)}%`);
        }
        // Send back to subagent
        await this.sendBackToSubagent(work, auditResult);
        this.storeAuditResult(auditResult);
     ...
     */
    async sendBackToSubagent(work, auditResult) {
     ...
     */
    async recordSuccessfulCompletion(work, auditResult) {
     ...
     */
    async persistAuditToMemory(auditResult) {
        try {
...
exports.PrincessAuditGate = PrincessAuditGate;
exports.default = PrincessAuditGate;
