...
exports.ComplianceGateManager = void 0;
const events_1 = require("events");
class ComplianceGateManager extends events_1.EventEmitter {
    constructor(thresholds) {
        super();
        this.complianceHistory = new Map();
        this.pot10Requirements = new Map();
     ...
     */
    async validateCompliance(artifacts, context) {
        const violations = [];
     ...
     * Extract compliance data from artifacts
     */
    async extractComplianceData(artifacts, context) {
        const data = {};
    ...
    async calculateComplianceMetrics(data) {
        ...
        const overallScore = (documentation.coverage * 0.20 +
     ...
     */
    calculateDocumentationMetrics(data) {
     ...
     */
    calculateCodeQualityMetrics(data) {
        return {
            complexity: {
     ...
     */
    calculateTraceabilityMetrics(data) {
     ...
     */
    async validatePOT10Requirements(data) {
        const violations = [];
     ...
     */
    checkPOT10Rule(ruleKey, requirement, code) {
        let actualValue;
        let passed;
        switch (ruleKey) {
            case 'complexity':
     ...
     */
    validateDocumentationCompliance(metrics) {
        const violations = [];
     ...
     */
    validateSecurityCompliance(metrics) {
        const violations = [];
     ...
     */
    validateTestingCompliance(metrics) {
        const violations = [];
     ...
     */
    validateTraceabilityCompliance(metrics) {
        const violations = [];
     ...
     */
    generateComplianceRecommendations(metrics, violations) {
        const recommendations = [];
     ...
     */
    storeComplianceHistory(metrics) {
     ...
     */
    extractCodeAnalysisData(artifacts) {
        return artifacts.reduce((acc, artifact) => ({
            ...acc,
            ...artifact.data
        }), {});
    }
    extractDocumentationData(artifacts) {
        return artifacts.reduce((acc, artifact) => ({
            ...acc,
            ...artifact.data
        }), {});
    }
    extractTestingData(artifacts) {
        return artifacts.reduce((acc, artifact) => ({
            ...acc,
            ...artifact.data
        }), {});
    }
    extractSecurityData(artifacts) {
        return artifacts.reduce((acc, artifact) => ({
            ...acc,
            ...artifact.data
        }), {});
    }
    extractTraceabilityData(artifacts) {
        return artifacts.reduce((acc, artifact) => ({
            ...acc,
            ...artifact.data
        }), {});
...
exports.ComplianceGateManager = ComplianceGateManager;
//# sourceMappingURL=ComplianceGateManager.js.map