...
exports.AutoRollbackSystem = void 0;
...
class AutoRollbackSystem {
     ...
     */
    async updateRollbackConfig(deploymentId, configUpdate) {
        const monitoredDeployment = this.monitoredDeployments.get(deploymentId);
        if (!monitoredDeployment) {
            throw new Error(`Deployment ${deploymentId} is not being monitored`);
        }
     ...
     * Create monitored deployment context
     */
    async createMonitoredDeployment(execution) {
        return {
            execution,
     ...
     */
    async initializeHealthMonitoring(monitoredDeployment) {
     ...
     */
    async initializeMetricsEvaluation(monitoredDeployment) {
     ...
     */
    async startMonitoringLoops(monitoredDeployment) {
     ...
     */
    async handleHealthFailure(monitoredDeployment, reason, severity) {
     ...
     */
    async handleMetricsThresholdExceeded(monitoredDeployment, trigger, metrics) {
     ...
     */
    async executeRollback(monitoredDeployment, trigger, reason) {
        const deploymentId = monitoredDeployment.execution.id;
        try {
     ...
     */
    async requestManualApproval(monitoredDeployment, trigger, reason) {
        const deploymentId = monitoredDeployment.execution.id;
     ...
     */
    async executeStrategySpecificRollback(monitoredDeployment) {
     ...
     */
    async executeBlueGreenRollback(monitoredDeployment) {
     ...
     */
    async executeCanaryRollback(monitoredDeployment) {
     ...
     */
    async executeRollingRollback(monitoredDeployment) {
     ...
     */
    async executeGenericRollback(monitoredDeployment) {
     ...
     */
    async notifyRollbackListeners(deploymentId, reason) {
...
exports.AutoRollbackSystem = AutoRollbackSystem;
// Supporting classes
class HealthMonitor {
    constructor(monitoredDeployment) {
        this.monitoredDeployment = monitoredDeployment;
        this.monitoring = false;
        this.healthFailureCallbacks = [];
    ...
    async checkHealth() {
        ...
        const healthResults = await Promise.all(environment.healthEndpoints.map(endpoint => this.checkEndpointHealth(endpoint)));
...
class MetricsEvaluator {
    ...
    async collectApplicationMetrics(namespace) {
    ...
    async collectInfrastructureMetrics(namespace) {
        try {
            // For Kubernetes environments
            if (process.env.KUBERNETES_SERVICE_HOST) {
                const orchestrator = new container_orchestrator_1.ContainerOrchestrator({ platform: 'kubernetes' });
    ...
    async collectBusinessMetrics(namespace) {
    ...
    async shouldTriggerRollback(trigger, metrics) {
        switch (trigger.type) {
            case 'error-rate':
...
//# sourceMappingURL=auto-rollback-system.js.map