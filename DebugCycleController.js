...
exports.DebugCycleController = void 0;
const events_1 = require("events");
class DebugCycleController extends events_1.EventEmitter {
     ...
     */
    async runDebugCycle(files, initialTestResult, context, config) {
        ...
        for (let i = 1; i <= this.maxIterations; i++) {
            ...
            const iteration = await this.performDebugIteration(i, files, currentTestResult, previousErrors, context, config);
        ...
        if (result.iterations.length > 0) {
            result.confidenceScore = result.iterations.reduce((sum, iter) => sum + iter.confidenceScore, 0) / result.iterations.length;
        }
        console.log(`\n[DebugCycle] COMPLETE`);
     ...
     */
    async performDebugIteration(iterationNumber, files, testResult, errors, context, config) {
        const iteration = {
            iterationNumber,
            timestamp: Date.now(),
        ...
        iteration.confidenceScore = this.calculateConfidence(iteration.errorsResolved.length, errors.length, iteration.fixesApplied.length);
     ...
     */
    async generateFixes(errors, files, testResult, config) {
        const fixes = [];
        // Categorize errors
     ...
     */
    async fixCompilationErrors(errors, files) {
        const fixes = [];
     ...
     * Fix runtime errors
     */
    async fixRuntimeErrors(errors, files, testResult) {
        const fixes = [];
        for (const error of errors) {
     ...
     * Fix test failures
     */
    async fixTestFailures(errors, files, testResult) {
        const fixes = [];
     ...
     */
    async fixPerformanceIssues(errors, files, testResult) {
        const fixes = [];
     ...
     * Fix security issues
     */
    async fixSecurityIssues(errors, files, testResult) {
        const fixes = [];
     ...
     */
    async generateGenericFixes(errors, files) {
        const fixes = [];
        for (const error of errors) {
     ...
     */
    async generateAggressiveFixes(errors, files) {
        const fixes = [];
     ...
     * Fix missing import
     */
    fixMissingImport(file, line, message) {
     ...
     * Fix syntax error
     */
    fixSyntaxError(file, line, message) {
        return {
            file,
            line,
            originalCode: '',
     ...
     * Fix type error
     */
    fixTypeError(file, line, message) {
        return {
            file,
            line,
            originalCode: '',
     ...
     */
    generatePatternBasedFix(error, file) {
     ...
     */
    async revalidate(files, context) {
     ...
     */
    categorizeErrors(errors, testResult) {
        ...
        if (compilationErrors.length > 0)
            categories.set('compilation', compilationErrors);
        if (runtimeErrors.length > 0)
            categories.set('runtime', runtimeErrors);
        if (testErrors.length > 0)
            categories.set('test', testErrors);
        if (performanceErrors.length > 0)
            categories.set('performance', performanceErrors);
        if (securityErrors.length > 0)
            categories.set('security', securityErrors);
        return categories;
    }
    /**
     * Calculate confidence in fixes
     */
    calculateConfidence(errorsResolved, totalErrors, fixesApplied) {
        if (totalErrors === 0)
            return 1;
...
exports.DebugCycleController = DebugCycleController;
exports.default = DebugCycleController;
