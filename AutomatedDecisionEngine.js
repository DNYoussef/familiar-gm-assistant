...
exports.AutomatedDecisionEngine = void 0;
const events_1 = require("events");
class AutomatedDecisionEngine extends events_1.EventEmitter {
    constructor(config) {
        super();
        this.decisionHistory = new Map();
        this.remediationResults = new Map();
     ...
     */
    async processDecisions(violations, metrics, context) {
        try {
     ...
     */
    analyzeQualityData(violations, metrics) {
     ...
     */
    extractCriticalMetrics(metrics) {
        return {
     ...
     * Assess risk factors
     */
    assessRiskFactors(violations, metrics) {
     ...
     */
    async planRemediation(violations, context) {
        const applicableStrategies = [];
        let totalEstimatedTime = 0;
        let overallSuccessProbability = 1;
        const rollbackPlan = ['git stash', 'git reset --hard HEAD'];
     ...
     */
    findRemediationStrategy(violation) {
        return this.config.remediationStrategies.find(strategy => violation.description?.toLowerCase().includes(strategy.violationType) ||
            violation.category === strategy.category);
    }
    /**
     * Plan escalation for serious issues
     */
    planEscalation(violations, decision) {
     ...
     */
    determineActions(decision, remediationPlan, escalation) {
        const actions = [];
        switch (decision) {
            case 'pass':
     ...
     */
    shouldAutoRemediate(violations, remediationPlan) {
     ...
     */
    storeDecisionHistory(gateId, result) {
     ...
     */
    async executeRemediationStrategy(strategy) {
...
exports.AutomatedDecisionEngine = AutomatedDecisionEngine;
//# sourceMappingURL=AutomatedDecisionEngine.js.map