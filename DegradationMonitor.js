...
exports.DegradationMonitor = void 0;
const ContextDNA_1 = require("./ContextDNA");
const ContextValidator_1 = require("./ContextValidator");
const GitHubProjectIntegration_1 = require("./GitHubProjectIntegration");
class DegradationMonitor {
    ...
    startMonitoring() {
        ...
        this.monitoringInterval = setInterval(() => this.performMonitoringCycle(), DegradationMonitor.MONITORING_INTERVAL);
    }
    /**
     * Stop monitoring
     */
     ...
     */
    async monitorTransfer(context, fingerprint, previousFingerprints) {
     ...
     */
    calculateCurrentDrift(current, previous) {
        if (previous.length === 0) {
            return 0;
        }
        const original = previous[0];
        return ContextDNA_1.ContextDNA.calculateDrift(original, current);
    }
    /**
     * Calculate drift rate (change per minute)
     */
    calculateDriftRate(agentPair, currentDrift) {
     ...
     */
    projectFutureDrift(currentDrift, driftRate) {
     ...
     */
    calculateTimeToThreshold(currentDrift, driftRate) {
        if (driftRate <= 0) {
     ...
     */
    generateAlert(metrics, fingerprint) {
        let level;
        let message;
     ...
     */
    async determineRecoveryAction(metrics, fingerprint, context) {
        ...
        if (metrics.currentDrift >= DegradationMonitor.CRITICAL_DRIFT) {
            ...
            const checkpointAvailable = await this.checkCheckpointAvailability(fingerprint.targetAgent);
     ...
     */
    async handleCriticalAgents(criticalAgents) {
    ...
    async executeRollback(action) {
        ...
        if (rollbackResult.success) {
            ...
            const validation = await this.validateRollbackContext(rollbackResult.context, rollbackResult.checksum);
            // Update Plane with rollback status
            await this.planeIntegration.updateTransferStatus(action.checkpointId, validation.valid ? 'recovered' : 'degraded', `Rollback executed: ${validation.valid ? 'successful' : 'validation failed'}`);
            return {
     ...
     */
    async executeReconstruction(action) {
        ...
        try {
            ...
            const validation = await this.validateReconstructedContext(reconstructedContext, action.targetAgent);
            // Create checkpoint for reconstructed context
            if (validation.valid) {
                await this.planeIntegration.createCheckpoint(action.targetAgent, reconstructedContext, validation.checksum, 'Context reconstruction recovery');
            }
            return {
                success: validation.valid,
     ...
     */
    async executeEscalation(action) {
        ...
        try {
            ...
            if (typeof globalThis !== 'undefined' && globalThis.mcp__plane__createIssue) {
                const escalationIssue = await globalThis.mcp__plane__createIssue({
                    projectId: this.planeIntegration.projectId,
     ...
     */
    async validateRollbackContext(context, checksum) {
        try {
            const currentChecksum = ContextDNA_1.ContextDNA.generateFingerprint(context, 'rollback', 'validation').checksum;
            return {
     ...
     */
    async gatherReconstructionSources(agent) {
        const sources = [];
        try {
     ...
     */
    async synthesizeContext(sources) {
        if (sources.length === 0) {
     ...
     * Validate reconstructed context
     */
    async validateReconstructedContext(context, agent) {
        try {
            const fingerprint = ContextDNA_1.ContextDNA.generateFingerprint(context, 'reconstruction', agent);
            return {
     ...
     */
    async logRecoveryAction(action, result, executionTime) {
        try {
            const logEntry = {
                action: action.type,
                agent: action.targetAgent,
...
exports.DegradationMonitor = DegradationMonitor;
DegradationMonitor.CRITICAL_DRIFT = 0.15; // 15% maximum allowed
DegradationMonitor.WARNING_DRIFT = 0.10; // 10% warning level
DegradationMonitor.MONITORING_INTERVAL = 10000; // 10 seconds
exports.default = DegradationMonitor;
