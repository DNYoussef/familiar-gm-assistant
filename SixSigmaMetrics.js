...
exports.SixSigmaMetrics = void 0;
const events_1 = require("events");
class SixSigmaMetrics extends events_1.EventEmitter {
     ...
     */
    async validateMetrics(artifacts, context) {
        const violations = [];
     ...
     */
    async extractMetricsData(artifacts, context) {
        const metricsData = {};
     ...
     */
    async calculateSixSigmaMetrics(metricsData) {
     ...
     */
    calculateDefectRate(metricsData) {
     ...
     */
    calculateProcessCapability(metricsData) {
        ...
        const cpk = Math.min((performanceTarget + tolerance - performanceMean) / (3 * performanceVariation), (performanceMean - (performanceTarget - tolerance)) / (3 * performanceVariation));
     ...
     * Calculate yield metrics
     */
    calculateYieldMetrics(metricsData) {
     ...
     */
    calculateSigmaLevel(defectRate, yield) {
        // Convert defect rate to sigma level
        let level;
        if (defectRate <= 3.4)
            level = 6;
        else if (defectRate <= 233)
            level = 5;
        else if (defectRate <= 6210)
            level = 4;
        else if (defectRate <= 66807)
            level = 3;
        else if (defectRate <= 308538)
            level = 2;
        else
            level = 1;
     ...
     */
    calculateQualityScore(defectRate, processCapability, yield, sigma) {
     ...
     */
    async validateCTQSpecifications(metricsData) {
        const ctqResults = [];
     ...
     */
    getActualValueForCTQ(ctqSpec, metricsData) {
        switch (ctqSpec.name) {
            case 'Code Coverage':
     ...
     */
    generateRecommendations(metrics, ctqValidation) {
        const recommendations = [];
     ...
     */
    storeHistoricalData(metrics, ctqValidation) {
     ...
     */
    extractPerformanceMetrics(performanceData) {
     ...
     */
    extractComplianceMetrics(complianceData) {
...
exports.SixSigmaMetrics = SixSigmaMetrics;
//# sourceMappingURL=SixSigmaMetrics.js.map